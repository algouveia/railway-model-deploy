...
from peewee import Model, IntegerField, FloatField, DateTimeField
...
feature_ranges = {
    "sku": (1124.0, 4735.0),
    "time_key": (20230103.0, 20500000.0),
...
########################################
# Feature Handling Setup
########################################

required_columns = {"sku", "time_key"}
# Example: Use training-time or domain-consistent defaults for optional features
default_values = {
    "sales_volume": 0.0,
    "chain_price": 0.0,
    "competitorA_campaign": 0.0,
    "competitorB_campaign": 0.0,
    "day_of_week": 0.0,
    "is_weekend": 0.0,
    "month": 1.0,
    "quarter": 1.0,
    ...
    "month_sin": 0.0,
    "month_cos": 1.0,
    "pvp_final_competitorA_ma7": 4.34,
    "pvp_final_competitorB_ma7": 3.99,
    "pvp_final_competitorA_ma14": 4.34,
    "pvp_final_competitorB_ma14": 3.99,
    "pvp_final_competitorA_ma30": 4.34,
    "pvp_final_competitorB_ma30": 3.99,
    "price_diff_A_vs_B": 0.0,
    "price_ratio_A_vs_B": 1.0,
    "price_diff_A_vs_chain": 0.0,
    "price_ratio_A_vs_chain": 1.0,
    # Add more as needed, matching your training defaults
}
...
def validate_price_request(req, require_actual=False):
    ...
    try:
        ...
        if require_actual:
            float(req['pvp_is_competitorA_actual'])
            float(req['pvp_is_competitorB_actual'])
    ...
    for col in columns:
        if col in req:
            try:
                ...
                raise ValueError(f"Invalid value for {col}: {e}")
        elif col in required_columns:
            missing_required.append(col)
        else:
            features_dict[col] = default_values.get(col, 0.0)
    if missing_required:
...
def forecast_prices():
    if not model_loaded:
        return jsonify({'error': f'Model not loaded: {model_error}'}), 503
    try:
...
def health_check():
    if model_loaded:
        ...
        return jsonify({'status': 'model not loaded', 'error': model_error}), 503
